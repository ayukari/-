...
| 2026-09-14 | **利用者の手元の端末**（機種未確認） | 20×14 の部屋・7人・家具29種、公開版（solo） | **「fps は全然問題ない」との報告** | 参考値。**0-5 の本丸（Core i5 級の内蔵GPU / VDI）の代替にはならない**。機種が分かり次第この行を埋める |
...
| # | 実装 | 対応 |
| --- | --- | --- |
| 1-1 | Google Workspace OIDC 認証、ドメイン検証、組織・メンバー — **いまは `?u=` の仮認証**。差し替え箇所は `wsServer.js` の1箇所に閉じてある | M-01, M-02 |
| 1-2 | ~~Realtime Gateway（WS、tick 10Hz、AOI フィルタ）~~ → **[server/src/adapters/wsServer.js](../../server/src/adapters/wsServer.js) に実装済み**（TTL プレゼンスは Redis 導入後） | [04 §4](./04-architecture.md) |
| 1-3a | ~~**論理レイヤー**と `SpaceRenderer` インターフェース~~ → **[client/src/world.js](../../client/src/world.js) に実装済み**。3D/2D が同じインターフェースを実装している | [04 §4.5](./04-architecture.md) |
| 1-2a | ~~Gateway の中核ロジック（tick / AOI / 差分抑制 / レート制限）~~ → **[server/](../../server/) に実装・テスト済み**（WebSocket アダプタは未着手） | [09](./09-protocol.md) |
| 1-4a | ~~サーバ権威の移動・着席・ノックの検証~~ → **[server/](../../server/) に実装・テスト済み（73件、うち通し試験8件）** | [09 §5](./09-protocol.md) |
| 1-3b | ~~**2D ビュー実装**~~ → **[client/src/render2d.js](../../client/src/render2d.js)**。PixiJS ではなく素の Canvas 2D で足りた（依存を1つ減らせた） | M-17 |
| 1-3c | ~~**3D ビュー実装**（Three.js、影なし、固定カメラ）~~ → **[client/src/render3d.js](../../client/src/render3d.js) が動作**。アバターは glTF、部屋（床・壁・天井・家具29種）は [client/src/props.js](../../client/src/props.js) でコードから生成（ダウンロード 0） | M-03, [04 §4.6](./04-architecture.md), [10 §5](./10-assets.md) |
| 1-3e | ~~**アセットパックの選定**~~ → **買わずに全部生成することにした。**アバターは [assets/avatar.blend.py](../../assets/avatar.blend.py)（**83パーツ・13万通り**・18ボーン・3クリップ・テクスチャ0）、部屋は [client/src/props.js](../../client/src/props.js)（**29種・ダウンロード0**）。**外部アセットは1枚も使っていない** | [10 §6](./10-assets.md)。自前生成なので商用可・改変可・**再配布可**を自動的に満たす |
| 1-3d | 解像度スケーリング（1.0x / 0.75x / 0.5x 自動調整） | M-16, [02 §6.4](./02-problem-solving.md) |
| 1-4b | ~~アバター移動と補間~~ → **実装済み**。自分は予測＋巻き戻し照合、他人は 10Hz→30fps の補間 | M-04 |
| 1-4 | サーバ権威の移動検証（速度上限・壁の衝突） | [04 §5.3](./04-architecture.md) T4 |
| 1-5 | ~~ステータス（自己申告4種）~~ → **実装済み**（他クライアントへ伝播することを通し試験で確認） | M-05 |
| 1-6 | 着席通話（会話単位の LiveKit room、トークン発行） | M-06, [04 §5.2](./04-architecture.md) |
| 1-7 | ~~ノック（承認・拒否・無反応）~~ → **実装済み**。無反応（15秒で「あとで」扱い）が既定 | M-07 |
...
- [x] ~~**WebGL を無効にしたブラウザで 2D ビューに落ちて動作する**~~ → プロトタイプで検証済み。
      実クライアントも `webglAvailable()` で自動切替（`?view=2d` で強制もできる）
- [x] ~~**同じ論理データで 3D ビューと 2D ビューが同一の空間・位置・会話を再現する**~~ →
      両者は `ClientWorld` を読むだけで、論理も通信も共有している
- [ ] **非通話時にブラウザのマイクインジケータが点灯していない**（[02 壁4](./02-problem-solving.md) の中核）
- [ ] UDP を塞いだネットワークで通話が成立する
- [ ] 非アクティブタブで CPU 使用率が実質 0
- [x] ~~座標・組織の偽装が通らないこと~~ → **サーバ側ロジックのテストで担保済み**（[server/](../../server/)）。
      SFU トークン発行と結合した状態での確認は、1-12 の実装後
- [ ] 座標を偽装したクライアントが、遠くの会話 room のトークンを取得できない（結合後の確認）
...
| # | 論点 | 期限 |
| --- | --- | --- |
| ~~O-1~~ | ~~選択肢 A（WorkAdventure ベース）/ B（ゼロから）~~ → **B（ゼロから）で決定（2026-09-13）**。3D を採用したことで、2Dタイルマップ前提の WorkAdventure をベースにする利点が大きく減ったため | ✅ 決定 |
