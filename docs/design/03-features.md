...
| M-03 | フロア表示 | タイルマップ上に自分と他人のアバターを描画。パン/ズーム |
...
| M-09 | プライベートルーム | 招待制 or ノック承認制。**非参加者にはメディアが配信されない** |
...
| M-16 | 軽量モード | Canvas2D / 15fps / アニメーション停止。低スペック端末・VDI 向け（[02 壁6](./02-problem-solving.md)） |
| M-17 | リストモード | 空間描画を捨て、メンバー一覧＋カード＋チャットのみ。極端に非力な環境向け |
| M-18 | 接続診断ツール | 導入前に UDP / TURN / WebSocket / 帯域 / 遅延を判定し、情シス向けレポートを出力（[02 壁5](./02-problem-solving.md)） |
| M-19 | 低帯域モード | 音声を 16kbps、位置同期を 3Hz に落とす |
| M-20 | テキストモード | 通話機能を使わず、空間・カード・チャットのみ。通話は既存の Zoom/Teams に任せる |
...
| アバターの課金カスタマイズ・装飾アイテム | 目新しさの追加は定着率を解かない（[02 壁2-H1](./02-problem-solving.md)） |
| 誰でも編集できるフロアエディタ | 運用が荒れる。v1 は管理者が JSON を入れる |
| ネイティブアプリ | ブラウザで足りる。**インストール不要であることがセキュリティ審査を通す最大の武器**（[02 壁4](./02-problem-solving.md)） |
| **3D 空間** | 業務PC・VDI で動かない。2D タイルマップ固定（[02 壁6](./02-problem-solving.md)）。**後から「3Dにしよう」と言い出せないよう明記する** |
...
| 分類 | 要件 |
| --- | --- |
| **軽さ** | 非アクティブタブで実質CPU 0。アクティブ非通話時 **10%未満**。タブのメモリ 200MB 以内。アイドル上り 10kbps 未満（[02 壁2-H3・壁6](./02-problem-solving.md)） |
| **描画** | **30fps 上限**（60fps は目指さない）。静止時は再描画しない（VDI の帯域対策）。WebGL 不可の環境では Canvas2D にフォールバック |
...
| プライバシー | 音声・映像は E2E ではなく SFU 経由（TLS/SRTP）。**非参加者へは配信経路自体が存在しない** |
| アクセシビリティ | キーボードのみで移動・着席・会話開始が可能。カードはスクリーンリーダー対応 |
