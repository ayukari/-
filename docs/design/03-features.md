...
| **外部 CDN からのフォント・ライブラリ読み込み** | セルフホストで「外部通信は認証だけ」と言えなくなる。**すべて自ホスト**（[審査パッケージ 01 §4](../security-review-pack/01-endpoints.md)） |
| 解析 SaaS・エラー収集 SaaS | 同上。依存を1つ増やすたびに許可リストが1行悪化する |
