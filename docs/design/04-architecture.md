...
| 空間描画 | **PixiJS**（WebGL、Canvas2Dフォールバック） | 数十のスプライトを 60fps で描くのに十分軽い。非可視時に停止しやすい | Phaser（ゲーム機能が過剰）、DOM（数十体で破綻） |
| マップ形式 | **Tiled (.tmj)** | 既存エディタで作れる。WorkAdventure も同形式で、資産と知見が流用できる | 独自JSON（エディタを自作する羽目になる） |
...
```mermaid
flowchart TD
    A["標準<br/>WebGL・30fps・歩行アニメあり"] -->|フレームタイム閾値割れ| B
    B["軽量モード<br/>Canvas2D・15fps・静止スプライト<br/>エフェクトなし・低解像度テクスチャ"] -->|WebGL不可 / 極端に非力| C
    C["リストモード<br/>空間描画なし<br/>メンバー一覧＋カード＋チャットの DOM 表示"]
```
