...
> 層の切り方（core / systems / ports / adapters）、状態の置き場所の3分割、
> オブジェクト種別の登録簿による拡張は [08. 拡張のしかた](./08-extensibility.md) にまとめた。

#### 4.5.1 表現を別の実行系に移すとき（Unity 版）— 移動の規則が2つになる

いま `core/movement.js` は**サーバとブラウザが同じファイルとして**読んでいる。
だから移動の規則はこの世に1つしかない。ここが予測と権威のずれを RTT 分に抑えている。

**Unity（C#）のクライアントは、このファイルを読めない。** 書き写すしかなく、
その瞬間に規則が2つになる。2つになると、片方を触ったときにもう片方が置き去りになり、
**「歩いていたら引き戻される」が常態化する**（これは [02 壁6](./02-problem-solving.md) の
縮退とは別の、もっと地味で直りにくい壊れ方である）。

書き写しは避けられないので、**入出力のほうを固定する。**
...
サーバが速度上限と壁の衝突を検証して確定させる。

> 具体的なバイト形式・検証規則・巻き戻し照合は
> **[09. ワイヤプロトコル仕様](./09-protocol.md)** で確定した。確定した座標だけがブロードキャストされ、
入室判定にもそれが使われる。
...
| メディア | LiveKit トークンは room 限定・短命。DTLS-SRTP。**音声は E2EE を既定 ON、プライベートルームは強制**（[07. 接続の安全性](./07-security.md)） |
...
> **経路ごとの暗号化・E2EE・鍵配布・攻撃面・限界は
> [07. 接続の安全性](./07-security.md) に分離した。**
> 要点: TURN リレーは中身を見られない。**SFU だけが唯一の弱点**であり、
> そこに E2EE・セルフホスト・プライベートルームでの E2EE 強制の3つを重ねる。
...
| # | 論点 | 判断の期限 |
| --- | --- | --- |
| ~~O-1~~ | ~~位置のバイナリプロトコルの具体的なフォーマット~~ → **[09. ワイヤプロトコル仕様](./09-protocol.md)** で確定（intent 10バイト / tick ヘッダ10バイト + 8バイト×人数） | ✅ 解決 |
