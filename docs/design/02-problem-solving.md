...
### 追記（2026-09-13）: この壁には、実在する答えがあった

[01 §3b](./01-market-research.md) で **gogh**（ambr・日本、Steam 高評価94%）を調べて分かったこと。

**gogh は「アバターと部屋の作り込みそのもの」を継続の理由にしている。**
髪型・服・小物・部屋の家具と色、貼れる画像。しかもそれを DLC として売っている。

これはわれわれにとって重要である。理由は、
**[00 原則](./00-overview.md) のせいで、定着の常套手段がほぼ全部使えない**から。

| よくある定着の手 | このプロダクトで使えるか |
| --- | --- |
| 通知で引き戻す | ✕ 原則4「急かさない」 |
| 連続ログイン記録・実績バッジ | ✕ 原則3「稼働を記録しない」 |
| 「誰が何時間いたか」を見せる | ✕ 同上 |
| ランキング | ✕ 同上 |
| **自分の見た目を育てる** | **○ 何にも反しない** |

**引き戻さずに戻ってきてもらう仕掛けとして、アバターは数少ない候補である。**
しかも他人に見られる場なので、着替える動機が一人用アプリより強い。

2026-09-13、これを受けて
[10 §4](./10-assets.md) のアバターを **プリセット18通り → パーツ83種・13万通り** に作り直した。
ただし **原則3は曲げない**。着せ替えの履歴も、誰が何を着ていたかも保存しない。

> ⚠ **まだ検証していない仮説である。** gogh は個人が自分のために入れるアプリで、
> 会社が配るツールで同じ動機が働くとは限らない。Phase 2 の「W2-H1: 目新しさが尽きる」の
> 検証項目に、**着替えの発生率**を入れる。
...
審査が長引く最大の原因は「聞かれてから調べる」ことである。

→ **[docs/security-review-pack/](../security-review-pack/) に作成済み**（2026-09-12）。

- [01. 通信先とポート一覧](../security-review-pack/01-endpoints.md) — 許可リストにコピペできる形式。**5行**
- [02. データフローと復号点](../security-review-pack/02-data-flow.md) — どこで誰が中身を見られるか
- [03. 保存データ一覧](../security-review-pack/03-stored-data.md) — 保持期間・削除・**保存しないもの**
- [04. 監査ログ仕様書](../security-review-pack/04-audit-log.md) — 残す/残さないの境界
- [05. 想定問答集](../security-review-pack/05-qa.md) — 26問への回答（未実装の項目も明記）

> **書いてみたら設計の穴が2つ見つかった。**
> ①Web フォントを外部 CDN から読んでいて、許可リストが2行増えていた（自ホストに変更）
> ②`PRIVATE_AREA_INVITED` の監査ログが「1年残る、誰と誰が一緒にいた記録」になっていた
> （招待先と被招待者を記録しない形に変更）
>
> **設計ドキュメントを読み返すだけでは見つからなかった。
...
> **決定（2026-09-13）: 3D を標準として採用する。**
> ただしこれは Phase 0 の実機ベンチ**の前**に下された決定であり、
> 「内蔵GPU / VDI で 30fps 出る」という確認は取れていない。
> したがって R-4 は解決ではなく **受容したリスク**であり、
> **縮退経路が唯一の担保**になる。前提条件は [10 §9](./10-assets.md) に列挙した。
> WebGL 非対応端末での 2D 自動起動は実装・検証済み。
>
