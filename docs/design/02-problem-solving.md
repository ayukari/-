...
| 壁6 | 「会社のポンコツPCで3D描画はキツい」 | 業務端末の性能で動かない |
...
### 前提の確認: **本プロダクトは 3D ではない**

まずこれが最大の回答になる。[04. §2](./04-architecture.md) の通り、
描画は **2D タイルマップ（PixiJS / WebGL、Canvas2D フォールバック）** である。3D は作らない。

- 3D の仮想オフィス（国内では RISA など）と比べて、負荷が桁違いに小さい
- Gather も 2D。このカテゴリで 2D が主流なのは、まさにこの理由である
- **3D を作らないことを [03. Won't](./03-features.md) に明記**し、後から誰かが「3Dにしよう」と言い出せないようにする

### 想定する最低スペック（ここを外すと全部が机上の空論になる）

```
Core i5-8250U 相当 / 内蔵GPU / メモリ 8GB / Windows 11 / Chrome
かつ Excel・Teams・ブラウザのタブ 15個が同時に開いている状態
```

これを **Phase 1 の受け入れ基準（DoD）** にし、実機で測る（[06. ロードマップ](./06-roadmap.md)）。

| 指標 | 目標値 |
| --- | --- |
| フレームレート | 30fps（60fps は目指さない） |
| CPU 使用率（アクティブ・非通話） | 10% 未満 |
| CPU 使用率（非アクティブタブ） | 実質 0 |
| タブのメモリ使用量 | 200MB 以内 |

**60fps を目指さない**のが要点である。アバターがタイル上を歩くだけの表現に 60fps は要らない。
30fps 上限にするだけで CPU 負荷はおよそ半分になる。

### 解1: 3段階の縮退

性能に応じて、描画の重さを段階的に落とす。

| モード | 内容 | 想定環境 |
| --- | --- | --- |
| 標準 | WebGL、30fps、歩行アニメーションあり | 一般的な業務PC |
| **軽量モード** | Canvas2D、15fps、アニメーション停止（静止スプライト）、エフェクトなし、テクスチャ低解像度 | 非力なPC、VDI |
| **リストモード** | **空間描画を完全に捨て、メンバー一覧＋フォーカスカード＋チャットの DOM 表示のみ** | 極端に非力な環境、モバイル |

ここでも **フォーカスカードが中核である設計が効いている。**
空間を丸ごと捨ててもプロダクトの価値が残る。空間が主役なら、この縮退は不可能だった。

### 解2: VDI・リモートデスクトップを明示的に想定する

見落とされがちだが、業務PCが **VDI（仮想デスクトップ）** であることは珍しくない。この環境では:

- WebGL が極端に遅い、あるいは使えない → **Canvas2D フォールバックが必須**
- 画面全体が動画として転送されるため、**常時再描画が回線を食う**（壁5に転移する）
- 対策: **静止時は再描画しない**。誰も動いていないフレームでは描画をスキップし、変化した領域だけ更新する

「動きがないときは1ピクセルも描き変えない」を実装方針として明記する。

### 解3: 性能を測って提案する（勝手に切り替えない）

- 起動後 30 秒間のフレームタイムを計測する
- 閾値を下回ったら **「軽量モードにしますか?」を1度だけ提案**する
- **自動で切り替えない。** [00. 原則2](./00-overview.md)「推定しない」の精神をここにも適用する
- 一度選んだ設定は端末ごとに記憶する

### 解4: メモリとアセットの上限を設計時に決める

- スプライトはアトラス1枚に統合し、テクスチャ総量に上限を設ける
- マップは可視領域のみ描画（カリング）
- フォントとアイコンを絞る
- **非アクティブタブで描画ループを完全停止**（[04. §4.3](./04-architecture.md)）
  → 8GB のPCで1日中裏に置いても邪魔をしない
...
| **2Dタイルマップ**（3Dを作らない） | — | 転送量が小さい | 負荷が桁違いに小さい |
...
| 軽量モード / リストモード | Phase 2 | 壁6・壁5。縮退経路 |
