 ...
 * 10 §4。1体の最重の組み合わせの上限。
 *
 * ★ 1,900 から引き上げてある。手足を「円錐＋球」の継ぎ足しから
 *   一本の掃引面に作り直し、角度で滑らかにした（かくかくを直した）ぶん、
 *   1体あたりの三角形が増えた。
 *   引き上げてよい根拠は下の「25人ぶん」のテスト:
 *   3,700 × 25 + 部屋 31,000 = 123,500 で、1フレーム 150,000 の予算に収まる。
 */
const MAX_TRIS = 3700;
/** 10 §1 のアバターの取り分 */
...
test('25人ぶんでも1フレームの三角形の予算に収まる', () => {
  // 10 §1: 1フレーム 150,000 未満。部屋の建材・家具・小物を 31,000 と見て引いた残り
  //   （実測は 25,354。床板・壁・天井の庇・家具29種を入れた値。見積りは余裕を持たせてある）
  const common = meshTris(glb(COMMON));
